...
export function hourFromTimeStr(str, defaultHour = 8) {
  if (str == null || typeof str !== 'string') return defaultHour;
  const h = parseInt(String(str).trim().split(':')[0], 10);
  return Number.isNaN(h) ? defaultHour : Math.max(0, Math.min(23, Math.floor(h)));
}
 ...
 */
/** First assignment for an hour (supports array or single value per hour). */
function getFirstForHour(assignments, hour) {
  const v = assignments[hour];
...
export function suggestLoadLightening(assignments, goals, maxSlots, energyModifier = 0) {
  ...
  const filled = SLOT_HOURS.filter((hour) => {
    const a = getFirstForHour(assignments, hour);
    ...
    for (const item of dayItems) {
      ...
      for (let h = 0; h < hours && slotIdx < slotHours.length; h++) {
        ...
        if (goal.type === 'routine') {
          ...
          dayAssignments[hour] = [isPriority
            ? { goalId: goal.id, priority: true }
            : goal.id];
        }
      }
    }
