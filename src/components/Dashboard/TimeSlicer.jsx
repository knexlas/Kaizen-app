...
import { toCanonicalSlotKey, slotKeyToMinutesSinceMidnight } from '../../services/schedulingConflictService';
...
const HOUR_START = 6;
const HOUR_END = 23;
/** Half-hour slots from 06:00 to 23:00 so 10:30 events display and block correctly. */
const HOURS = Array.from(
  { length: (HOUR_END - HOUR_START) * 2 + 1 },
  (_, i) => {
...
export function getGoalIdFromAssignment(a) {
...
/** True if the hour/slot has no assignments (exact key lookup). */
function isSlotEmpty(assignments, hour) {
  return getAssignmentsForHour(assignments, hour).length === 0;
}

/** All assignments that start in the 30-min row (rowKey e.g. "10:00" = 10:00–10:30). Returns { slotKey, assignment, indexInSlot }[] sorted by time. */
function getAssignmentsForRow(assignments, rowKey) {
  if (!assignments || !rowKey) return [];
  const rowStart = slotKeyToMinutesSinceMidnight(rowKey);
  if (rowStart == null) return getAssignmentsForHour(assignments, rowKey).map((a, i) => ({ slotKey: rowKey, assignment: a, indexInSlot: i }));
  ...
  for (const [key, raw] of Object.entries(assignments)) {
    ...
    if (startMins == null || startMins < rowStart || startMins >= rowEnd) continue;
    const list = Array.isArray(raw) ? raw : raw != null ? [raw] : [];
    list.forEach((a, i) => out.push({ slotKey: toCanonicalSlotKey(key) ?? key, assignment: a, indexInSlot: i }));
  }
  out.sort((a, b) => {
    const ma = slotKeyToMinutesSinceMidnight(a.slotKey) ?? 0;
    const mb = slotKeyToMinutesSinceMidnight(b.slotKey) ?? 0;
    return ma !== mb ? ma - mb : a.indexInSlot - b.indexInSlot;
  });
  return out;
}
...
export function getDurationMinutesFromAssignment(assignment, goals) {
...
function TimeSlot({
  ...
  const firstAssignment = list[0]?.assignment ?? list[0] ?? null;
  ...
  const slotMins = (slotH ?? 0) * 60 + (slotM ?? 0);
  const nowMins = now ? now.getHours() * 60 + now.getMinutes() : 0;
  const isCurrentSlot = now && slotMins <= nowMins && nowMins < slotMins + 30 && slotH >= hourStart && slotH <= hourEnd;
  const currentMinutePercent = isCurrentSlot ? ((nowMins - slotMins) / 30) * 100 : 0;
  const timeLabel = isCurrentSlot ? `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}` : '';
    ...
    <div ref={setNodeRef} className="relative flex items-center gap-3 py-1.5">
      ...
      {onEmptySlotClick && (
        <button
          ...
          className="shrink-0 w-8 h-8 inline-flex items-center justify-center rounded-lg border border-stone-300 bg-white text-stone-600 hover:bg-stone-100 hover:text-stone-800 focus:outline-none focus:ring-2 focus:ring-moss-500/40"
          aria-label={`Add item to ${hour}`}
          title={`Add item to ${hour}`}
        >
          +
        </button>
      )}
      ...
      >
        ...
        {!isEmpty ? (
          (() => {
            const slotDurations = list.map((item) => getDurationMinutesFromAssignment(item.assignment, goals));
            const totalDuration = slotDurations.reduce((s, d) => s + d, 0);
            const SLOT_BASE_HEIGHT_PX = 84;
          ...
          <div className="flex flex-col gap-1 p-1 overflow-auto" style={{ minHeight: SLOT_BASE_HEIGHT_PX, height: slotHeightPx }}>
            ...
            {list.map((item, index) => {
              ...
              const indexInSlot = item.indexInSlot;
              const goalId = getGoalIdFromAssignment(assignment);
              ...
              const isStandaloneEvent = assignment && typeof assignment === 'object' && assignment.type === 'event';
              ...
              const durationMinutes = getDurationMinutesFromAssignment(assignment, goals);
              const heightPx = totalDuration > 0 ? Math.max(22, Math.round((durationMinutes / totalDuration) * slotHeightPx)) : slotHeightPx;
              const isMicroTask = durationMinutes < 20;
              const slotBgForChip = isRecovery
                ? 'bg-stone-100 border-stone-300 text-stone-600'
                : isStandaloneEvent
                  ? 'bg-sky-100 border-sky-300 text-sky-900'
                  : routineSession || isRoutineTemplate
                    ? 'bg-slate-200 border-slate-300 text-stone-800'
                    : isCriticalMass
                      ? 'bg-orange-200 border-orange-300'
                      : 'bg-moss-200 border-moss-500/50 text-stone-800';
              ...
              return (
                ...
                >
                    ...
                    {routineSession ? (
                      isMicroTask ? (
                        <div className="flex items-center gap-1.5 min-w-0 w-full">
                          ...
                          {onStartFocus && goal && (
                            <button type="button" onClick={() => onStartFocus(goal.id, slotKey, undefined, assignment?.subtaskId)} className="shrink-0 ml-auto flex items-center gap-0.5 px-1.5 py-0.5 rounded font-sans text-[10px] bg-stone-800 text-stone-50 hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-moss-500/50"><span aria-hidden>▶</span></button>
                          )}
                        </div>
                      ) : (
                      <>
                        <div className={`flex items-center justify-between gap-2 ${isFullyHarvested ? 'line-through text-stone-400 opacity-60' : ''} ${isPriority ? 'pl-5' : ''}`}>
                          ...
                          {onStartFocus && goal && (
                            <button type="button" onClick={() => onStartFocus(goal.id, slotKey, undefined, assignment?.subtaskId)} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded font-sans text-xs bg-stone-800 text-stone-50 hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-moss-500/50">
                      ...
                      isMicroTask ? (
                        <div className="flex items-center gap-1.5 min-w-0 w-full">
                          <span className="font-sans text-[10px] text-sky-700 shrink-0" aria-hidden>{durationMinutes}m</span>
                          <span className="font-sans text-xs font-medium text-sky-900 truncate">{assignment.title ?? 'Event'}</span>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-sans text-sm font-medium text-sky-900 truncate">{assignment.title ?? 'Event'}</span>
                          </div>
                          <span className="font-sans text-xs text-sky-700 mt-0.5">{durationMinutes}m</span>
                        </>
                      )
                    ) : goal ? (
                      isMicroTask ? (
                        <div className="flex items-center gap-1.5 min-w-0 w-full">
                          ...
                          {onStartFocus && !isFullyHarvested && (
                            <button type="button" onClick={() => onStartFocus(goal.id, slotKey, slotRitualTitle ?? undefined, assignment?.subtaskId)} className="shrink-0 ml-auto flex items-center gap-0.5 px-1.5 py-0.5 rounded font-sans text-[10px] bg-stone-800 text-stone-50 hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-moss-500/50"><span aria-hidden>▶</span></button>
                          )}
                          {onStartFocus && isFullyHarvested && (
                            <button type="button" onClick={(e) => { triggerHarvestConfetti(e); onHarvestedClick?.(); onStartFocus(goal.id, slotKey, slotRitualTitle ?? undefined, assignment?.subtaskId); }} className="shrink-0 ml-auto flex items-center gap-0.5 px-1.5 py-0.5 rounded font-sans text-[10px] bg-moss-600/90 text-stone-50 font-medium hover:bg-moss-600 focus:outline-none focus:ring-2 focus:ring-moss-500/50" title="Harvested"><span aria-hidden>✔</span></button>
                          )}
                        </div>
                      ) : (
                      <>
                        ...
                        <div className={`flex flex-col gap-0.5 ${isCriticalMass ? 'pl-5' : ''} ${isPriority && !isCriticalMass ? 'pl-5' : ''}`}>
                          <div className={`flex items-center justify-between gap-2 pr-24 ${isFullyHarvested ? 'line-through text-stone-400 opacity-60' : ''}`}>
                            ...
                            {onStartFocus && !isFullyHarvested && (
                              <button type="button" onClick={() => onStartFocus(goal.id, slotKey, slotRitualTitle ?? undefined, assignment?.subtaskId)} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded font-sans text-xs bg-stone-800 text-stone-50 hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-moss-500/50"><span aria-hidden>▶</span><span>Start</span></button>
                            )}
                            {onStartFocus && isFullyHarvested && (
                              <button type="button" onClick={(e) => { triggerHarvestConfetti(e); onHarvestedClick?.(); onStartFocus(goal.id, slotKey, slotRitualTitle ?? undefined, assignment?.subtaskId); }} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded font-sans text-xs bg-moss-600/90 text-stone-50 font-medium hover:bg-moss-600 focus:outline-none focus:ring-2 focus:ring-moss-500/50" title="Harvested — tap to focus again"><span aria-hidden>✔</span><span>Harvested</span></button>
                            )}
                          </div>
                          ...
                          {isFullyHarvested && onStartFocus && (
                            <button type="button" onClick={(e) => { triggerHarvestConfetti(e); onHarvestedClick?.(); onStartFocus(goal.id, slotKey, slotRitualTitle ?? undefined, assignment?.subtaskId); }} className="self-start mt-1 font-sans text-xs text-moss-700 hover:text-moss-800 underline underline-offset-1 focus:outline-none focus:ring-2 focus:ring-moss-500/40 rounded">Over-water (add more)</button>
                          )}
                          {estimatedMins > 0 && (
                  ...
                  {typeof onRemoveSlotItem === 'function' && (
                    <button type="button" onClick={() => onRemoveSlotItem(slotKey, indexInSlot)} className="shrink-0 p-1.5 rounded text-stone-400 hover:text-stone-600 hover:bg-stone-200 focus:outline-none focus:ring-2 focus:ring-moss-500/50" aria-label={`Remove from ${slotKey}`}>×</button>
                  )}
                </motion.div>
              );
            })}
    ...
    <div className="mb-4">
      ...
      >
        ...
        {list.map(({ assignment, originalIndex }, index) => {
          ...
          const routineSession = isRoutineSession(assignment);
          const goal = goalId ? goals?.find((g) => g.id === goalId) : null;
          const title = routineSession
            ? (assignment.title ?? goal?.title ?? 'Routine')
            : (goal?.title ?? (assignment && typeof assignment === 'object' && assignment.title) ?? 'Task');
          const firstUncompleted = goal?.milestones?.find((m) => !m.completed);
          const milestoneTitle = firstUncompleted?.title ?? firstUncompleted?.text;
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg border shrink-0 ${completedClasses} ${
                routineSession ? 'bg-slate-200 border-slate-300' : isStandaloneEvent ? 'bg-sky-100 border-sky-300' : 'bg-moss-200 border-moss-500/50'
              } text-stone-800`}
            >
...
/** Planned minutes for this goal from current week assignments (kaizen: by goalId; routine: by routine sessions). Iterates all slot keys so minute-precision slots are included. */
...
/** Planned minutes for a routine goal: sum duration of all slots with parentGoalId === goalId. Iterates all slot keys. */
        ...
        <div className="flex items-center gap-1 min-w-0">
          ...
          {onStartFocus && (
            <button type="button" onClick={(e) => { e.stopPropagation(); onStartFocus(goal.id, null, displayTitle, undefined); }} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded font-sans text-xs bg-moss-600 text-stone-50 hover:bg-moss-700 focus:outline-none focus:ring-2 focus:ring-moss-500/50" aria-label={`Do ${displayTitle} now`}>
              <span aria-hidden>▶️</span><span>Start now</span>
            </button>
          )}
...
function summarizeDayAssignments(dayAssign, goals) {
  return summarizeDayPlan(dayAssign, goals);
}
...
function WeekView({ weekAssignments, goals, onDayClick, onPlanWeek, planningWeek, weekPreview, onConfirmWeekPlan, onDiscardWeekPlan, calendarEvents }) {
  ...
  const weekStats = useMemo(() => {
    ...
    dates.forEach((d) => {
      ...
      Object.keys(da).forEach((hourKey) => {
        for (const a of getAssignmentsForHour(da, hourKey)) {
          totalMinutes += getDurationMinutesFromAssignment(a, goals);
        }
      });
    });
    const totalPlanned = Math.round((totalMinutes / 60) * 10) / 10;
    return { totalPlanned };
    ...
    <div className="space-y-3">
      ...
      <div className="min-w-0 overflow-x-auto pb-2 -mx-1 px-1">
        ...
        {dates.map((dateStr, i) => {
          ...
          const totalMinutes = summary.reduce((s, x) => s + (x.minutes || 0), 0);
          ...
          return (
            <button
              ...
              className={`flex flex-col rounded-xl p-2.5 min-h-[150px] border transition-all text-left focus:outline-none focus:ring-2 focus:ring-moss-500/40 group min-w-0 ${
                isToday
              ...
              {summary.length === 0 ? (
                ...
                <div className="flex flex-col gap-0.5 flex-1">
                  {summary.slice(0, 3).map((s) => (
                    <div
                      key={s.goalId}
                      className={`px-2 py-1 rounded text-[11px] leading-snug font-sans ${
                        s.type === 'routine'
                      ...
                      title={`${s.title} - ${s.minutes || 0}m`}
                    >
                      <span className="font-medium">{s.title}</span>
                      <span className="ml-1 text-[10px] opacity-75">{Math.max(1, Math.round(s.minutes || 0))}m</span>
                    </div>
                  ))}
                  {summary.length > 3 && (
...
function MonthPlanView({ weekAssignments, goals, onDayClick, monthlyRoadmap, onPlanMonth, planningMonth, calendarEvents = [], planMonthLabel = '✨ Plan My Month', planMonthBusyLabel = '✨ Mochi is looking at your calendar...' }) {
  ...
  const monthStats = useMemo(() => {
    ...
    cells.forEach((dateStr) => {
      ...
      Object.keys(da).forEach((hourKey) => {
        for (const a of getAssignmentsForHour(da, hourKey)) {
          dayMinutes += getDurationMinutesFromAssignment(a, goals);
        }
      });
      if (dayMinutes > 0) plannedDays++;
      totalMinutes += dayMinutes;
    });
    const totalHours = Math.round((totalMinutes / 60) * 10) / 10;
    return { totalHours, plannedDays, daysInMonth };
  }, [cells, weekAssignments, goals, daysInMonth]);
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {cells.map((dateStr, i) => {
          ...
          const slotCount = Object.keys(dayAssign).filter((key) => key !== 'anytime').length;
          const visibleEvents = dedupeCalendarEventsForDate(calendarEvents, dayAssign, dateStr);
          const weather = getWeekWeather(visibleEvents, dateStr);
          const eventCount = visibleEvents.length;
          const isToday = dateStr === todayStr;
          const isPast = dateStr < todayStr;
          const dayNum = new Date(dateStr + 'T12:00:00').getDate();
          const weekdayLabel = new Date(dateStr + 'T12:00:00').toLocaleDateString(undefined, { weekday: 'short' });
          const density = slotCount === 0 ? 'none' : slotCount <= 3 ? 'light' : slotCount <= 6 ? 'medium' : 'heavy';
          const taskPreview = summarizeDayAssignments(dayAssign, goals).slice(0, 2);
          const previewItems = [
            ...visibleEvents.slice(0, 1).map((event) => ({
              key: `event-${event.id ?? event.title ?? dateStr}`,
              title: event.title ?? event.summary ?? 'Event',
              tone: 'bg-sky-100 text-sky-800',
            })),
            ...taskPreview.slice(0, Math.max(0, 2 - Math.min(1, visibleEvents.length))).map((item) => ({
              key: `task-${item.goalId}-${item.title}`,
              title: item.title,
              tone: item.type === 'routine' ? 'bg-slate-100 text-slate-700' : item.type === 'event' ? 'bg-sky-100 text-sky-800' : 'bg-moss-100 text-moss-800',
            })),
          ];
          return (
            <button
              ...
              type="button"
              onClick={() => onDayClick?.(dateStr)}
              className={`relative min-h-[88px] sm:min-h-[104px] flex flex-col items-stretch justify-start rounded-lg font-sans text-xs transition-all focus:outline-none focus:ring-2 focus:ring-moss-500/30 group p-1.5 ${
                isToday
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <span className="text-left text-[11px] uppercase tracking-wide text-stone-400">{weekdayLabel}</span>
                <span>{dayNum}</span>
              </div>
              <div className="flex gap-px mt-1">
                ...
                {eventCount > 0 && (
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    eventCount >= 4 ? 'bg-sky-500' : eventCount >= 2 ? 'bg-sky-400' : 'bg-sky-300'
                  }`} />
                )}
              </div>
              <div className="mt-2 flex-1 space-y-1 text-left">
                {previewItems.length > 0 ? previewItems.map((item) => (
                  <span key={item.key} className={`block truncate rounded px-1.5 py-1 text-[10px] leading-tight ${item.tone}`}>
                    {item.title}
                  </span>
                )) : (
                  <span className="block text-[10px] italic text-stone-400">Open day</span>
                )}
              </div>
            </button>
          );
...
function DayDetailModal({ dateStr, dayAssignments, goals, onClose, onSwitchToDay, calendarEvents = [], hours = HOURS }) {
  ...
  const dayCalEvents = dedupeCalendarEventsForDate(calendarEvents, dayAssignments, dateStr);
...
function TimeSlicer({
  ...
  const [seedPickerSearch, setSeedPickerSearch] = useState('');
  ...
  const hoursArray = useMemo(
    () =>
      Array.from(
        { length: Math.max(0, (hourEnd - hourStart) * 2 + 1) },
        (_, i) => {
          const totalMins = hourStart * 60 + i * 30;
          const h = Math.floor(totalMins / 60);
          const m = totalMins % 60;
          return `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`;
        }
  ...
  const suppressNowPrimaryActions = zenMode && suppressPrimaryActionsInZen;
  const effectiveOnStartFocus = suppressNowPrimaryActions ? undefined : onStartFocus;
  ...
  useEffect(() => {
    if (!seedPickerTargetHour) return;
    setActiveSeedTab('quick');
    setSeedPickerSearch('');
  ...
  useEffect(() => {
    if (zenMode && viewMode !== 'day') setViewMode('day');
  }, [zenMode, viewMode]);
  ...
  const filledTimes = hoursArray.filter((h) => getAssignmentsForRow(assignments, h).length > 0);
  ...
  const todayTasks = useMemo(() => {
    ...
    for (const hour of hoursArray) {
      for (const { assignment: a } of getAssignmentsForRow(assignments, hour)) {
  ...
  const filledSpoonTotal = useMemo(() => {
    ...
    for (const h of hoursArray) {
      for (const { assignment: a } of getAssignmentsForRow(assignments, h)) {
  ...
  const handlePrioritizeSelectTask = (goal) => {
    if (!goal?.id) return;
    const firstEmpty = hoursArray.find((h) => getAssignmentsForRow(assignments, h).length === 0);
    if (!firstEmpty) {
  ...
  const normalizeEventDuration = useCallback((value) => {
    const mins = Number(value);
    if (!Number.isFinite(mins)) return 30;
    return Math.max(5, Math.min(180, Math.round(mins)));
  }, []);

  const buildStandaloneEventAssignment = useCallback((title, duration) => ({
    id: crypto.randomUUID?.() ?? `evt-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
    type: 'event',
    title: (title || '').trim() || 'Event',
    duration: normalizeEventDuration(duration),
    isFixed: true,
  }), [normalizeEventDuration]);

  const handleAddQuickEventToSlot = useCallback(() => {
    if (!seedPickerTargetHour || !hoursArray.includes(seedPickerTargetHour)) return;
    const value = buildStandaloneEventAssignment(quickEventTitle, quickEventDuration);
    applyAssignment(seedPickerTargetHour, value);
    setSeedPickerTargetHour(null);
    setQuickEventTitle('');
    setQuickEventDuration('30');
    setSeedPickerSearch('');
    setActiveSeedTab('all');
    window.dispatchEvent(new CustomEvent('kaizen:toast', { detail: { message: 'Event added to your day.' } }));
  }, [seedPickerTargetHour, hoursArray, buildStandaloneEventAssignment, quickEventTitle, quickEventDuration, applyAssignment]);
  ...
  const handleDragEnd = (event) => {
    ...
    const targetWasEmpty = getAssignmentsForRow(assignments, time).length === 0;
  ...
  const todayStr = useMemo(() => localISODate(), []);
  const activeDayKey = editingDate || todayStr;
  const mergedWeekAssignments = useMemo(
    () => ({ ...(weekAssignments ?? {}), [activeDayKey]: assignments ?? {} }),
    [weekAssignments, activeDayKey, assignments]
  );
  ...
  const handleSwitchToInspectedDay = useCallback(() => {
    ...
    if (dateStr && dateStr !== todayStr) {
      ...
      const dayData = mergedWeekAssignments[dateStr] ?? {};
    ...
    setViewMode('day');
  }, [inspectedDate, todayStr, mergedWeekAssignments, isControlled, safeOnAssignmentsChange]);
  ...
  const handleBackToToday = useCallback(() => {
    ...
    setEditingDate(null);
    if (isControlled) safeOnAssignmentsChange(mergedWeekAssignments[todayStr] ?? {});
  }, [editingDate, isControlled, assignments, safeOnAssignmentsChange, mergedWeekAssignments, todayStr, saveDayPlanForDate]);
  ...
  const currentNextItem = useMemo(() => {
    ...
    const items = [];
    for (const hour of hoursArray) {
      for (const { slotKey, assignment: a } of getAssignmentsForRow(assignments, hour)) {
        const slotMins = slotKeyToMinutesSinceMidnight(slotKey) ?? 0;
        const goalId = getGoalIdFromAssignment(a);
        const goal = safeGoals.find((g) => g.id === goalId);
        const assignmentTitle = a && typeof a === 'object' ? a.title : null;
        const isStandaloneEvent = a && typeof a === 'object' && a.type === 'event';
        if (isStandaloneEvent && !goal) continue;
        items.push({ hour: slotKey, goalId, goal, slotMins, ritualTitle: getRitualTitleFromAssignment(a), subtaskId: getSubtaskFromAssignment(a)?.id, assignmentTitle });
      }
    }
    if (items.length === 0) return null;
    const sorted = [...items].sort((x, y) => x.slotMins - y.slotMins);
    const upcoming = sorted.filter((i) => i.slotMins >= nowMins);
    const first = upcoming.length > 0 ? upcoming[0] : sorted[0];
    const title = first.ritualTitle || first.goal?.title || first.assignmentTitle;
    ...
    <div className="bg-stone-50 rounded-xl border border-stone-200 p-6 min-w-0">
      <div className="relative z-50 flex items-center justify-between gap-2 mb-4 flex-wrap">
        <div className="flex items-center gap-3">
          <h2 className="font-serif text-stone-900 text-lg">{zenMode ? 'Now' : 'Schedule'}</h2>
          {!zenMode && (
            ...
            {!suppressNowPrimaryActions ? (
              <>
                <button
                  ...
                  aria-label="Plan today (gentle priorities)"
                >
                  🎯 Plan today
                </button>
                <button
                  ...
                  onClick={(e) => {
                    ...
                    const dayEvents = [];
                    for (const h of hoursArray) {
                      const rowItems = getAssignmentsForRow(assignments, h);
                      for (let i = 0; i < rowItems.length; i++) {
                        const { slotKey, assignment: a } = rowItems[i];
                        const [hourNum, minNum] = slotKey.split(':').map(Number);
                        const gid = getGoalIdFromAssignment(a);
                        const goal = safeGoals.find((g) => g.id === gid);
                        const durationMins = getDurationMinutesFromAssignment(a, safeGoals);
                        const startTime = new Date(dateStr + 'T12:00:00');
                        startTime.setHours(hourNum ?? 0, minNum ?? 0, 0, 0);
                        const endTime = new Date(startTime.getTime() + durationMins * 60 * 1000);
                        dayEvents.push({ id: `kz-${slotKey}-${i}`, title: goal?.title || 'Kaizen Task', start: startTime, end: endTime });
                      }
                    }
              ...
              <button
                ...
                className="shrink-0 px-3 py-1.5 rounded-lg border border-moss-200 bg-moss-50 font-sans text-sm text-moss-800 hover:bg-moss-100 focus:outline-none focus:ring-2 focus:ring-moss-400/40 transition-colors"
              >
                Open planner
              </button>
            )}
      ...
      {viewMode === 'week' && (
        <WeekView
          weekAssignments={mergedWeekAssignments}
          goals={safeGoals}
      ...
      {viewMode === 'day' && !hideCapacityOnMobile && (
        <SpoonBattery
          ...
          onLightenLoad={!suppressNowPrimaryActions && isOverCapacity ? handleLightenLoad : undefined}
        />
      )}

      {viewMode === 'day' && zenMode && (
          ...
          <div className="p-5 rounded-2xl border-2 border-moss-400 bg-moss-50/90 shadow-md">
            ...
            {currentNextItem ? (
              <div className="flex flex-wrap items-center gap-3">
                ...
                {effectiveOnStartFocus && currentNextItem.goal && (
                  <button
                    type="button"
                    onClick={() => effectiveOnStartFocus(currentNextItem.goal.id, currentNextItem.hour, currentNextItem.ritualTitle ?? undefined, currentNextItem.subtaskId)}
      ...
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        {viewMode === 'day' && !suppressNowPrimaryActions && todayUncompletedTasks.length > 0 && !hasAnyPrioritizedToday && (
          <div className="mb-4">
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); handleOpenPrioritize(); }}
              className="w-full py-2.5 px-3 rounded-lg font-sans text-xs border border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-400/40 transition-colors"
              aria-label="Pick one task for me"
            >
              Pick one task for me
            </button>
          </div>
        )}
        ...
        <AnytimePoolSection
          ...
          goals={safeGoals}
          onStartFocus={effectiveOnStartFocus}
          ...
          <div className="flex gap-3 max-h-[70vh] md:max-h-[420px] overflow-y-auto overflow-x-hidden rounded-lg">
            ...
            <div className="flex-1 relative min-h-0">
              ...
              {(() => {
                ...
                const dayEvts = dedupeCalendarEventsForDate(safeCalendarEvents, assignments, targetDate);
              ...
              {hasNoAssignments && (
                <div className="flex flex-col items-center justify-center w-full text-center p-8 opacity-80 animate-in fade-in duration-500" aria-live="polite">
                  ...
                  <p className="text-stone-500 text-sm mb-3">Rest in your garden, or plan one next step.</p>
                  {onOpenPlanner && (
                    <button
                      type="button"
                      onClick={onOpenPlanner}
                      className="px-3 py-1.5 rounded-lg font-sans text-xs font-medium border border-moss-300 text-moss-700 hover:bg-moss-50 focus:outline-none focus:ring-2 focus:ring-moss-500/40"
                    >
                      Plan today
                    </button>
                  )}
              ...
              <div className="space-y-0">
                {hoursArray.map((hour) => (
                  <TimeSlot
                    key={hour}
                    hour={hour}
                    assignmentsList={getAssignmentsForRow(assignments, hour)}
                    goals={goals}
      ...
      {inspectedDate && (
        <DayDetailModal
          ...
          dayAssignments={mergedWeekAssignments[inspectedDate] ?? {}}
          goals={safeGoals}
          ...
          >
            ...
            >
              ...
              <div className="p-4 max-h-[60vh] overflow-y-auto space-y-4">
                <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-stone-100 border border-stone-200">
                  <button
                    type="button"
                    onClick={() => setActiveSeedTab('quick')}
                    className={`py-2 px-3 rounded-lg font-sans text-xs font-medium transition-colors ${
                      activeSeedTab === 'quick'
                ...
                {(activeSeedTab === 'quick' || !activeSeedTab) && (
                  <div className="rounded-xl border border-sky-200 bg-sky-50/70 p-3">
                    <label className="font-sans text-xs font-medium text-sky-800 block mb-1.5" htmlFor="quick-event-title">
                      Event name
                    </label>
                    <input
                      ...
                      placeholder="e.g. Team check-in, call mom, dentist"
                      className="w-full p-2.5 rounded-lg border border-sky-200 bg-white font-sans text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-sky-400/50"
                      autoFocus
                    />
                    <div className="mt-2.5 flex items-center gap-2">
                      <label className="font-sans text-xs text-stone-600" htmlFor="quick-event-duration">
                        Duration
                      </label>
                      <input
                        ...
                        onChange={(e) => setQuickEventDuration(e.target.value)}
                        onBlur={() => setQuickEventDuration(String(normalizeEventDuration(quickEventDuration)))}
                        className="w-20 p-2 rounded-lg border border-sky-200 bg-white font-sans text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-sky-400/50"
                      />
                      <span className="font-sans text-xs text-stone-500">min</span>
                      <button
                        type="button"
                        onClick={handleAddQuickEventToSlot}
                        className="ml-auto py-2 px-3 rounded-lg bg-sky-600 text-white font-sans text-sm font-medium hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500/50 transition-colors"
                      >
                        Add event
                      </button>
                    </div>
                ...
                {activeSeedTab === 'all' && (() => {
                  const searchLower = (seedPickerSearch || '').trim().toLowerCase();
                  const ritualsFiltered = todayRitualEntries.filter(({ goal, ritual }) => {
                    const g = (goal?.title || '').toLowerCase();
                    const r = (ritual?.title || '').toLowerCase();
                    return !searchLower || g.includes(searchLower) || r.includes(searchLower);
                  });
                  const bankFiltered = searchLower
                    ? safeGoalBank.filter((g) => {
                        const title = (g?.title || '').toLowerCase();
                        const type = (g?.type || '').toLowerCase();
                        const domain = (g?.domain || '').toLowerCase();
                        const project = (g?._projectName || '').toLowerCase();
                        return title.includes(searchLower) || type.includes(searchLower) || domain.includes(searchLower) || project.includes(searchLower);
                      })
                    : safeGoalBank;
                  const kz = bankFiltered.filter((g) => g.type !== 'routine' && g.type !== 'vitality' && !g._projectGoal);
                  const rt = bankFiltered.filter((g) => g.type === 'routine');
                  const pr = bankFiltered.filter((g) => g._projectGoal);
                  const vt = bankFiltered.filter((g) => g.type === 'vitality');
                  const sections = [
                  ...
                  return (
                    <>
                      <input
                        type="text"
                        placeholder="Search existing seeds..."
                        value={seedPickerSearch}
                        onChange={(e) => setSeedPickerSearch(e.target.value)}
                        className="w-full p-2.5 rounded-lg border border-stone-200 bg-stone-100 font-sans text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-moss-400/50"
                        aria-label="Search existing seeds"
                      />
                      {ritualsFiltered.length > 0 && (
                        <div>
                          <h3 className="font-sans text-xs font-medium text-amber-800 mb-2">Today&apos;s Rituals</h3>
                          <div className="flex flex-col gap-2">
                            {ritualsFiltered.map(({ goal, ritual }) => (
                              <button
                      ...
                      {sections.length === 0 ? (
                        <div><p className="font-sans text-xs text-stone-400 py-2">No matching goals yet.</p></div>
                      ) : (
                        sections.map((s) => (
                          <div key={s.id}>
