...
const PREFERENCE_OPTIONS = [
  { id: 'morning', label: 'Morning Heavy', emoji: '🌅' },
  { id: 'afternoon', label: 'Afternoon Heavy', emoji: '☀️' },
...
function GoalCreator({ open, onClose, onSave, initialTitle = '', initialSubtasks = [], existingRoutineGoals = [], existingVitalityGoals = [] }) {
  ...
  const [liquidPreference, setLiquidPreference] = useState('balanced');
  // Vitality: Health/Metric
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 pb-8" noValidate>
              ...
              {/* Vitality: Metric name, current, target, direction */}
              {goalType === 'vitality' && (
                <div className="space-y-4 p-4 rounded-xl bg-sky-50/80 border border-sky-200 mb-6">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Weight, Body Fat, Sleep Score"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block font-sans text-sm font-medium text-stone-600 mb-1">Target Value</label>
                      <input
                        type="number"
                        step="any"
                ...
                <>
                  ...
                  {scheduleMode === 'liquid' && (
                    <div className="space-y-4 p-4 rounded-xl bg-stone-100/80 border border-stone-200 mb-6">
                      ...
                      <div>
                        <label className="block font-sans text-xs font-medium text-stone-600 mb-2">Preference</label>
                        <div className="flex flex-wrap gap-2">
                          {PREFERENCE_OPTIONS.map((opt) => (
                            <button
                              key={opt.id}
                              type="button"
                              onClick={() => setLiquidPreference(opt.id)}
                              className={`px-3 py-2 rounded-lg font-sans text-sm transition-colors ${
                                liquidPreference === opt.id ? 'bg-moss-600 text-stone-50' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'
                              }`}
                            >
                              {opt.emoji} {opt.label}
                            </button>
