...
export default function ProficiencyArc({ goal, effectiveTotalMinutes }) {
  ...
  const hasEstimates = !!estimates && typeof estimates === 'object';
  const safeEstimates = hasEstimates ? estimates : {};

  const beginner = Math.max(0, Number(safeEstimates.beginner) || 20);
  const intermediate = Math.max(beginner, Number(safeEstimates.intermediate) || 100);
  const mastery = Math.max(intermediate, Number(safeEstimates.mastery) || 10000);
  ...
  const rank = currentHours >= mastery
    ? { label: 'Mastery', sub: null }
    : currentHours >= intermediate
      ? { label: 'Intermediate', sub: null }
      : currentHours >= beginner
        ? { label: 'Beginner', sub: null }
        : { label: 'Novice', sub: null };

  const hoursToNext = currentHours >= mastery
    ? null
    : currentHours >= intermediate
      ? Math.max(0, mastery - currentHours)
      : currentHours >= beginner
        ? Math.max(0, intermediate - currentHours)
        : Math.max(0, beginner - currentHours);
